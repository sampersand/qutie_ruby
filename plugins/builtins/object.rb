...
class QT_Object
  ...
  # qt methods
    ...
    # conversion
      ...
      def qt_to_type(_env); QT_Type.new( self.class ) end
    ...
    # operators 
      # access
        ...
        def _peek(env, amnt=1)
          qt_get(QT_Number.new( amnt ), env, type: QT_Symbol.new( :STACK ) )
        end
        def _next(env, amnt=1)
          res = _peek(amnt)
          qt_del(QT_Number.new( amnt ), env, type: QT_Symbol.new( :STACK ) )
          res
        end
        def _stackeach(_env)
      ...
      #3##
        ...
        def __bi_method(right, env)
          ...
          return res unless res._missing?
          throw(:ERROR, QTE_MethodMissing.new(env, callername, self, right))
          fail
        end
