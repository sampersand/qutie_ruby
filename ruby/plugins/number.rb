module Number
  ...
  def next_base!(stream, _, _)
    ...
    case res[1]
    when 'b' then res += stream.next(amnt: 1) while stream.peek?(/[01]/, len: 1)
    when 'o' then res += stream.next(amnt: 1) while stream.peek?(/[0-7]/, len: 1)
    when 'd' then res += stream.next(amnt: 1) while stream.peek?(/[0-9]/, len: 1)
    when 'x' then res += stream.next(amnt: 1) while stream.peek?(/[0-9A-F]/i, len: 1)
  ...
  def next_int!(stream, universe, parser)
    ...
    parser.catch_EOF(universe) {
      num += stream.next(amnt: 1) while stream.peek?(/\d/, len: 1)
      nil
    }
    num
  end
  ...
  def next_float!(stream, universe, parser)
    ...
    parser.catch_EOF(universe) {
      ...
      return start + stream.next(amnt: 1) + next_int!(stream, universe, parser) || '0'
      nil
    }
    start
