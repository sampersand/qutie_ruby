module Comment
  ...
  def next_single!(stream)
    ...
    stream.next(amnt: 1) until stream.peek?(SINGLE_END)
    stream.next(amnt: 1)
    :retry
  end
  ...
  def next_multi!(stream)
    ...
    stream.next(amnt: 1) until stream.peek?(MULTI_LINE_END) # this will fail inside strings, but that's ok C does as well.
    stream.next(amnt: MULTI_LINE_END)
    :retry
  end
